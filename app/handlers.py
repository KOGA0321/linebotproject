...
def handle_message(event: MessageEvent):
    ...
    response = client.chat.completions.create(
        model=model,
        messages=[{"role":"system","content":"あなたは共感力に優れたメンタルケアAIです。相手の気持ちに寄り添い、優しく親身になって相談にのってください。堅苦しい敬語は控えて、友達のようなフラットな口調で話しましょう。短く簡潔に返答し、一方的に解決策を押し付けるのではなく、一緒に原因を探るようにやりとりしてください。相手の発言に感情的なワードが含まれていれば、それに合わせた対応を心がけてください。そして、もし日本語ではない言語でメッセージが送られてきたら送られてきた言語と同じ言語で返してください。"}]
                 + context_messages
    )
